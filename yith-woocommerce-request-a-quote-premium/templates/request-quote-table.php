...
elseif ( count( $raq_content ) === 0 ) :
	if ( empty( $notices ) || 0 === $notices ) {
		...
		?>
<a class="button " href="<?php echo esc_url( $shop_url_after_sent_email ); ?>"><?php echo esc_html( $label_return_to_shop_after_sent_email ); ?></a>
<?php
	}
else :
	?>
...
<a class="button return_to_shop_url" href="<?php echo esc_url( $shop_url ); ?>"><?php echo esc_html( $label_return_to_shop ); ?></a>
<?php endif; ?>
...
<form id="yith-ywraq-form" name="yith-ywraq-form" method="post">
  ...
  <table class="shop_table cart  <?php echo esc_attr( $responsive_class ); ?>" id="yith-ywrq-table-list">
    <thead>
      <tr>
        ...
        <th class="product-name" colspan="<?php echo $show_thumbnail ? '2' : '1'; ?>"><?php esc_html_e( 'Product', 'yith-woocommerce-request-a-quote' ); ?></th>
    ...
    <tbody>
      ...
      <tr class="<?php echo esc_attr( apply_filters( 'yith_ywraq_item_class', 'cart_item', $raq_content, $key ) ); ?>" data-wapo_parent_key="<?php echo esc_attr( $raq['yith_wapo_parent_key'] ); ?>" <?php echo esc_attr( apply_filters( 'yith_ywraq_item_attributes', '', $raq_content, $key ) ); ?>>
        ...
        <td class="product-name" data-title="<?php esc_attr_e( 'Product', 'yith-woocommerce-request-a-quote' ); ?>">
          ...
          <span class="mobile-price"><?php echo wp_kses_post( $show_single_price ? $price : $subtotal ); ?></span>
          <?php endif; ?>
        </td>
        <?php if ( $show_single_price ) : ?>
        <td class="product-price" data-title="<?php esc_attr_e( 'Price', 'yith-woocommerce-request-a-quote' ); ?>">
          <?php
        ...
        <?php endif; ?>
        <?php if ( $show_line_total ) : ?>
        <td class="product-subtotal" data-title="<?php esc_attr_e( 'Subtotal', 'yith-woocommerce-request-a-quote' ); ?>">
          <?php
      ...
      <tr class="<?php echo esc_attr( apply_filters( 'yith_ywraq_item_class', 'cart_item', $raq_content, $key ) ); ?>" <?php echo esc_attr( apply_filters( 'yith_ywraq_item_attributes', '', $raq_content, $key ) ); ?>>
        ...
        <td class="product-quantity" data-title="<?php esc_attr_e( 'Quantity', 'yith-woocommerce-request-a-quote' ); ?>">
          <?php
								$quantidademinima = get_field('quantidade_minima',  $product_id);
								...
								if ( $_product->is_sold_individually() ) {
									...
									$product_quantity = woocommerce_quantity_input(
										array(
											...
											'min_value'   => apply_filters( 'ywraq_quantity_min_value', $quantidademinima, $_product ),
      ...
      <?php
				if ( $total_tax > 0 && 'incl' !== $tax_display_list && apply_filters( 'ywraq_show_taxes_quote_list', false ) ) :
					...
					?>
      <tr>
        <td colspan="<?php echo esc_attr( $colspan ); ?>" data-colspan-mobile="<?php echo esc_attr( $mobile_colspan ); ?>"></td>
      ...
      <?php endif; ?>
      <tr>
        <th colspan="<?php echo esc_attr( $colspan ); ?>" data-colspan-mobile="<?php echo esc_attr( $mobile_colspan ); ?>" data-colspan="<?php echo esc_attr( $colspan ); ?>"></th>
        <th class="raq-totals-label">
  ...
  <div class="update-list-wrapper"><input type="submit" class="button" name="update_raq" value="<?php echo esc_attr( get_option( 'ywraq_update_list_label' ) ); ?>">
  </div>
